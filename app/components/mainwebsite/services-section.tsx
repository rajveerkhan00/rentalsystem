    ...
    <section className="relative py-24 bg-black overflow-hidden">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="relative text-center mb-24">
          ...
          <p className="text-pink-500 font-bold mb-4 tracking-[0.4em] uppercase text-xs">Unmatched Coverage</p>
          <h2 className="text-4xl md:text-6xl font-bold text-white text-balance tracking-tight leading-none mb-6">
            AIRPORT <span className="text-transparent bg-clip-text bg-gradient-to-r from-pink-500 via-purple-500 to-pink-500 animate-gradient-x">TRANSFERS</span>
          </h2>
          <div className="w-24 h-1 bg-gradient-to-r from-pink-500 to-purple-600 mx-auto mb-8 rounded-full shadow-[0_0_20px_rgba(236,72,153,0.5)]" />
          ...
          {airports.map((airport) => (
            <div
              ...
              className={`group relative p-10 rounded-[2.5rem] transition-all duration-700 border ${airport.featured
                ? "bg-gradient-to-br from-white/10 to-white/5 border-pink-500/50 shadow-[0_20px_50px_rgba(236,72,153,0.1)]"
                : "bg-white/5 border-white/10 hover:border-pink-500/40"
                } backdrop-blur-xl overflow-hidden hover:shadow-[0_0_50px_rgba(236,72,153,0.15)] hover:-translate-y-2 perspective-1000`}
            >
              {/* Spotlight Glow Effect */}
              <div className="absolute -inset-24 bg-gradient-to-tr from-pink-500/10 via-purple-500/10 to-transparent opacity-0 group-hover:opacity-100 blur-3xl transition-opacity duration-700" />

              <div className="relative z-10">
                <div
                  className={`w-20 h-20 rounded-3xl flex items-center justify-center mb-8 transition-all duration-700 group-hover:scale-110 group-hover:rotate-[10deg] ${airport.featured ? "bg-gradient-to-br from-pink-500 to-purple-600 shadow-2xl shadow-pink-500/50" : "bg-white/10 border border-white/10 shadow-xl"
                    }`}
                >
                  <Plane
                    className={`w-10 h-10 ${airport.featured ? "text-white" : "text-pink-500 group-hover:text-pink-400"} transition-colors`}
                  />
                  {/* Neon Ring Effect */}
                ...
                <h3 className="text-2xl font-bold mb-6 text-white group-hover:text-pink-400 transition-colors tracking-tight">{airport.name}</h3>
                <p
                ...
                {airport.link && (
                  <button className="relative group/btn overflow-hidden px-6 py-3 rounded-xl bg-white/5 border border-white/10 text-pink-500 font-bold flex items-center gap-3 transition-all duration-500 hover:bg-pink-500 hover:text-white hover:shadow-lg hover:shadow-pink-500/40">
